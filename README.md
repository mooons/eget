...
This fork of `zyedidia/eget` adds automatic macOS app-bundle installation while
keeping the existing CLI-binary workflow intact. On macOS, if `eget owner/repo`
selects a `.dmg`, `.zip`, or `.tar*` release asset that contains one or more
`.app` bundles, Eget installs those apps into `/Applications` by default with
no extra flags, installs all discovered apps when more than one is present, and
still behaves like upstream for normal CLI binary releases. Use `--to` to pick
another app destination, or use `--download-only` or `--file` to bypass the
automatic app-install path and keep manual extraction semantics.

[![Go Report Card](https://goreportcard.com/badge/github.com/mooons/eget)](https://goreportcard.com/report/github.com/mooons/eget)
[![Release](https://img.shields.io/github/release/mooons/eget.svg?label=Release)](https://github.com/mooons/eget/releases)
...
eget owner/repo
eget -f eget.1 mooons/eget
eget zachjs/sv2v
eget https://go.dev/dl/go1.17.5.linux-amd64.tar.gz --file go --to ~/go1.17.5
eget --all --file '*' ActivityWatch/activitywatch
```

On macOS, if `eget owner/repo` selects a release asset that contains one or
more `.app` bundles, those apps are installed into `/Applications` by default.

# How to get Eget

Before you can get anything, you have to get Eget. If you want this fork's
macOS app-bundle support, or you already have this fork installed and want to
upgrade it, use `eget mooons/eget`.

### Quick-install script

This upstream bootstrap script installs upstream `zyedidia/eget`, not this fork.
Use the fork release assets below if you want automatic macOS app-bundle
installation.
...
`brew install eget` currently installs upstream `zyedidia/eget`, not this fork.
...
`choco install eget` currently installs upstream `zyedidia/eget`, not this
fork.
...
[releases](https://github.com/mooons/eget/releases) page.
...
```
git clone https://github.com/mooons/eget
cd eget
make build # or go build (produces incomplete version information)
```

If you specifically want upstream instead of this fork, install it with:
...
(requires pandoc). You can also use `eget` to download the man page from this
fork: `eget -f eget.1 mooons/eget`.
